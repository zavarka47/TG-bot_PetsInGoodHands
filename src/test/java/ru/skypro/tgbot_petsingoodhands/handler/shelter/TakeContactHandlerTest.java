...
public class TakeContactHandlerTest {

    @InjectMocks
    private TakeContactHandler takeContactHandler;
    ...
    public static void initializationResource() throws URISyntaxException, IOException {
        String callbackQuery = Files.readString(Path.of(
                TakeContactHandlerTest.class.getClassLoader().getResource("callbackQuery.json").toURI()));
    ...
    public void handleUpdateTest(){
        ...
        takeContactHandler.handleUpdate(update);

        ArgumentCaptor<Long> chatIdCaptor = ArgumentCaptor.forClass(Long.class);
        ArgumentCaptor<String> textCaptor = ArgumentCaptor.forClass(String.class);

        verify(messages).sendMessageWithMarkdown(chatIdCaptor.capture(), textCaptor.capture());

        Long chatId = chatIdCaptor.getValue();
        String text =  textCaptor.getValue();

        Assertions.assertEquals(chatId, update.callbackQuery().from().id());
        Assertions.assertTrue(text.contains("Введите свои данные в формате"));
