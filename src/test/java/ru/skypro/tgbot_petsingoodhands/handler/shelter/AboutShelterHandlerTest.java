...
import ru.skypro.tgbot_petsingoodhands.handler.animal.FirstMeetRulesHandlers;
import ru.skypro.tgbot_petsingoodhands.listener.UpdateListener;
...
public class AboutShelterHandlerTest {

    @InjectMocks
    private AboutShelterHandler aboutShelterHandler;
    ...
    public static void initializationResource() throws URISyntaxException, IOException {
        String callbackQuery = Files.readString(Path.of(
                AboutShelterHandlerTest.class.getClassLoader().getResource("callbackQuery.json").toURI()));
    ...
    public void handleUpdateTest(){
        ...
        when(shelter.getAbout()).thenReturn("Живодёрня");

        aboutShelterHandler.handleUpdate(update);
        ...
        Assertions.assertTrue(text.contains("Живодёрня"));
