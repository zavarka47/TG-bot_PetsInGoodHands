...
public class WorkHourShelterHandlerTest {

    @InjectMocks
    private WorkHourShelterHandler workHourShelterHandler;
    ...
    public static void initializationResource() throws URISyntaxException, IOException {
        String callbackQuery = Files.readString(Path.of(
                WorkHourShelterHandlerTest.class.getClassLoader().getResource("callbackQuery.json").toURI()));
    ...
    public void handleUpdateTest(){
        ...
        when(shelter.getAddress()).thenReturn("work hour");

        workHourShelterHandler.handleUpdate(update);
        ...
        Assertions.assertTrue(text.contains("work hour"));
